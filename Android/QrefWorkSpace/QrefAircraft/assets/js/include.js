...
var reachability = QrefInterface.isConnected();
//Initialize
...
$(document).ready(function() {
	...
	$.ajax = function(obj) {
		...
		request.ontimeout = function() {
			QrefInterface.log("Timeout Happened!");
			
			if(obj.error) {
				if(obj.error) {
					obj.error(request.statusText, request.responseText);
				}
        ...
        request.onreadystatechange = function(){//Call a function when the state changes.
            ...
            QrefInterface.log("state = " + request.readyState);

            if (request.readyState == 4) {

                if (request.status == 200 || request.status == 0) {

                    QrefInterface.log("*" + request.responseText + "*");
        ...
        if(obj.contentType) {
			...
			else if(obj.contentType.indexOf('form') > -1) {
				if(typeof obj.data == 'object') {
					data = '';
	...
	AppObserver.set('reachable', reachability);
	ChecklistObserver.set('canCheck', QrefInterface.getCanCheck());
...
function loadCache() {
	...
	while(QrefInterface.hasNextChecklistVersionInfo()) {
		try {
			var version = QrefInterface.getNextChecklistVersionInfo();
			
			if(version != undefined) {
				var vi = JSON.parse(version);
...
function DataLoaded() {
	...
	AppObserver.load();
    if(QrefInterface.deviceHasChecklists()) {
