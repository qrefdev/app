    ...
    purchaseTap:function (element, e, data) {
        ...
        if (ProductDetailsObserver.product) {
            if (ProductDetailsObserver.product.userOwnsProduct) {
                ...
                QrefInterface.purchase(ProductDetailsObserver.product.androidProductIdentifier);
            }
        }
        else {
...
var MenuObserver = new zimoko.Observable({
    ...
    checkingTap:function (element, e, data) {
        ...
        if (ChecklistObserver.canCheck) {
            ...
            QrefInterface.setCanCheck(false);
        }
        else {
            checkbox.checked = true;
            ChecklistObserver.set('canCheck', true);
            QrefInterface.setCanCheck(true);
        }
    },
    ...
    open:function () {
        if (!$('.menu').hasClass('slided')) {
            ...
            AppObserver.set('reachable', QrefInterface.isConnected());
        }
    },
    close:function () {
        if ($('.menu').hasClass('slided')) {
...
var AppObserver = new zimoko.Observable({
    ...
    ajaxResponse: function(response) {
    	if(response) {
			if(typeof this.ajaxObj.success == 'function') {
				if(this.ajaxObj.dataType == 'json')
					this.ajaxObj.success(JSON.parse(response));
				else
					this.ajaxObj.success(response);
			}
		}
		else {
			if(typeof this.ajaxObj.error == 'function') {
				this.ajaxObj.error();
			}
		}
    },
