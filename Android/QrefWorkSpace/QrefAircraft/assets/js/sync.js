...
function SyncProcessor() {
    ...
    this.syncOneServer = function(checklist) {
        ...
        if(checklist != undefined && AppObserver.token != '' && AppObserver.token != undefined)
		{	
			AppObserver.setDeviceName(QrefInterface.getDeviceName());
	...
	*/
	...
	this.syncVersionInfoToPhone = function(versions, callback) {
		...
		function process(index, item) {
			...
			var filename = item._id + '.qvi';
			
			setTimeout(function() {
				QrefInterface.saveChecklistVersionInfo(stringifiedJson, filename);
			}, 20);
	...
	this.syncToPhone = function(lists, callback) {
		...
		if(lists != undefined && !self.syncing)
		{
            ...
            function process(index, item) {
				...
				setTimeout(function() {
					QrefInterface.saveChecklist(stringifiedJson, filename);
				}, 20);
