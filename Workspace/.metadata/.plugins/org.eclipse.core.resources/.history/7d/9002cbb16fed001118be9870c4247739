...
function scroll(direction, distance, duration) {
	if(hasContainer && isScrollable)
	{
		var extraScroll = distance * duration + ((0.5 * (duration * duration)) / 2);
			...
			case LEFT:
				...
				if(offscreen > 0)
				{
					if(extraScroll <= offscreen && offset > -offscreen)
					{
						...
						$container.animate({left: offset + "px"}, 500);
					}
					else
					{
						$container.animate({left: -offscreen + "px"}, 500);
					}	
				}
			...
			case UP:
				...
				if(offscreen > 0)
				{
					if(extraScroll <= offscreen && offset > -offscreen)
					{
						offset = offset - extraScroll;
						if(offset < -offscreen) offset = -offscreen;
						$container.animate({top: offset + "px"}, 500);
					}
					else
					{
						$container.animate({top: -offscreen + "px"}, 500);
					}
				}
				break;
			case DOWN:
