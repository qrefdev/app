...
function ShowDuplicateList(records)
{
	...
	{
		$('#dupChklist_Select').append('<option value=' + i + '>Version: ' + records[i].version +'</option>');
	}
...
{
	g_checklist = checklistResponse.records[$('#dupChklist_Select').val()]
	LoadObjects(checklistResponse.records[$('#dupChklist_Select').val()]);
...
{
	$("#ChecklistName").html(selectedmfg.name + " " + selectedmodel.name + " Version(" + selectedVersion + ")");
...
function CheckIn()
{
	if(g_checklist != "")
	{
		...
		g_checklist.preflight.length = 0;
		g_checklist.takeoff.length = 0;
		g_checklist.landing.length = 0;
		g_checklist.emergencies.length = 0;
