...
function Register() {
	...
	$.ajax({
		type: "post",
		data: register,
		url: "http://localhost:8080/services/rpc/auth/registerAccount",
		success: function(data) {
			var response = data;
...
function Signin() {
	...
	$.ajax({
		type: "post",
		data: signin,
		url: "http://localhost:8080/services/rpc/auth/login",
		success: function(data) {
			var response = data;
	...
	this.populate = function(status) {
		...
		for(var i = 0; i < Checklist.products.length; i++)
		{
			html += '<li data-id="' + Checklist.products[i]._id + '"><div class="heading">' + Checklist.products[i].airplaneChecklist.manufacturer.name +
					'</div><div class="subheading">' + Checklist.products[i].airplaneChecklist.model.name + '</div></li>';
		}
	...
	this.updateArea = function() {
		...
		else if(this.currentArea == "dashboard")
		{
			Checklist.load(function(status) {
				if(status == Checklist.CACHELOADED || status == Checklist.ONLINELOADEDAFTERCACHE || status == Checklist.ONLINEONLYLOADED)
				{
					MyProducts.reset();
					MyProducts.populate();
				}
...
function CheckListHandler() {
	...
	this.load = function(updateCallback) {
		...
		{	
			...
			if(updateCallback)
				updateCallback(this.CACHELOADED);
			...
			if(online)
			{
				this.onlineLoad(function(success) {
					if(success) {
						...
						if(updateCallback)
							updateCallback(self.ONLINELOADEDAFTERCACHE);
					}
					else
					{
						if(updateCallback)
							updateCallback(self.ONLINEFAILEDAFTERCACHE);
					}
				});
			}
		}
			...
			if(online)
			{
				this.onlineLoad(function(success) {
					if(success) {
						...
						if(updateCallback)
							updateCallback(self.ONLINEONLYLOADED);
					}
					else
					{
						if(updateCallback)
							updateCallback(self.ONLINEONLYFAILED);
					}
				});
			}
			else
