...
//Container for automatically scrolling if parent div has overflow: hidden
var $container = $element.find(".container");

//Check for container and scroll
var hasContainer = ($container.length > 0) ? true : false;
var isScrollable = ($element.css("overflow") == "hidden") ? true : false;
...
this.resetScroll = function() {
	if(hasContainer && isScrollable)
	{
		$container.css({top: "0px", left: "0px"});
	}
...
function scroll(direction, distance, duration) {
	if(hasContainer && isScrollable)
	{
		var extraScroll = (distance * duration + ((0.5 * (duration * duration)) / 2)) / 1000;
		...
		switch(direction) {
			case LEFT:
				...
				if(offscreen > 0)
				{
					if(extraScroll <= offscreen && offset > -offscreen)
					{
						...
						if(mouseDown)
							...
							$container.animate({left: offset + "px"}, 300);
					}
					else
					{
						if(mouseDown)
							...
							$container.animate({left: -offscreen + "px"}, 300);
					}	
				}
			...
			case RIGHT:
				...
				if(offscreen > 0)
				{
					if(extraScroll <= offscreen && offset < 0)
					{
						offset = offset + extraScroll;
				...
				if(offscreen > 0)
				{
					if(extraScroll <= offscreen && offset > -offscreen)
					{
						...
						if(mouseDown)
							...
							$container.animate({top: offset + "px"}, 300);
					}
					else
					{
						if(mouseDown)
							...
							$container.animate({top: -offscreen + "px"}, 300);
					}
				}
				break;
			case DOWN:
