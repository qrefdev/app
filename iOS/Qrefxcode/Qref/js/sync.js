...
function SyncProcessor() {
  ...
  this._sync = function(localItems, serverItems, updateDataSource, refreshChecklist) {
    ...
    setTimeout(function() {
      ...
      async.each(serverItems, function(serverItem, feCb) {
        var localItem = _.find(localItems, function(v) {
          return v._id == serverItem._id;
        });
          ...
          if (!existingVersionInfo) {
            ...
            self.evaluateChecklistVersions(localItem, existingVersionInfo, serverItem, syncTime, function(status, gapInfo) {
              if (status != 0) {
                var conflictObj = {
                  localItem: localItem,
                  localVersionInfo: existingVersionInfo,
                  serverItem: serverItem,
                  serverVersionInfo: self.formatVersionInfoEx(serverItem),
                  syncTime: syncTime,
                  gapInfo: gapInfo,
                  status: status
                };
        ...
        for (var i = 0; i < localItems.length; i++) {
          if (localItems[i].isDeleted == false) {
            var found = _.find(serverItems, function(z) {
              return z._id == localItems[i]._id;
            });
  ...
  this.getDeviceNameFromVersionInfo = function(versionInfo, serialNumber) {
    var ksn = _.find(versionInfo.knownSerialNumbers, function(v) {
  ...
  this.formatDate = function(usec) {
    var dt = new Date();
    dt.setTime(usec);
    return this.padNumber(dt.getMonth() + 1) + '-' + this.padNumber(dt.getDate()) + '-' + dt.getFullYear() + ' ' + this.padNumber(dt.getHours()) + ':' + this.padNumber(dt.getMinutes()) + ':' + this.padNumber(dt.getSeconds());
  };
  ...
  this.evaluateChecklistVersions = function(localItem, localVersionInfo, serverItem, syncTime, callback) {
    ...
    if (localVersionInfo.currentSerialNumber == serverVersionInfo.currentSerialNumber) {
      ...
      var localKsns = _.sortBy(localVersionInfo.knownSerialNumbers, function(v) {
        return v.serialNumber;
      });
      var serverKsns = _.sortBy(serverVersionInfo.knownSerialNumbers, function(v) {
        return v.serialNumber;
      });
      ...
      for (var z = 0; z < ksnCount; z++) {
        ...
        if (loKsn.serialNumber == seKsn.serialNumber) {
          ...
          gapInfo = {
            loIndex: z,
            seIndex: z,
            loSn: loKsn.serialNumber,
            seSn: seKsn.serialNumber,
            mKsn: mDelta,
            loKsn: loKsn,
            seKsn: seKsn
          };
  ...
  this.forceUpdateVersionInfo = function(chklst, versionInfo) {
    if (chklst.currentSerialNumber) {
      ...
      if (chklst.knownSerialNumbers) {
        var ksn = _.find(chklst.knownSerialNumbers, function(v) {
          return v.serialNumber == chklst.currentSerialNumber;
        });

        if (ksn) {
          versionInfo.knownSerialNumbers.push(ksn);
        } else {
          versionInfo.knownSerialNumbers.push({
            serialNumber: chklst.currentSerialNumber,
            deviceName: 'SYSTEM'
          });
        }
      } else {
  ...
  this.updateVersionInfo = function(newVersion, deviceName, versionInfo) {
    ...
    var ksn = _.find(versionInfo.knownSerialNumbers, function(v) {
      return v.serialNumber == newVersion;
    });
  ...
  this.formatVersionInfo = function(chklst) {
    ...
    if (chklst.currentSerialNumber && chklst.knownSerialNumbers) {
      var ksn = _.find(chklst.knownSerialNumbers, function(item) {
        return item.serialNumber == chklst.currentSerialNumber;
      });
